...
class Shill(Base):
    ...
    async def search(self):
        ...
        search = await self.new.get_by_label("Search and explore").is_visible()
        if search:
            await self.new.get_by_label("Search and explore").click()
        else:
            await self.new.goto('https://x.com/explore')
    ...
    async def execute(self):
        await self.search()
        count = 0
        count_range = random.choice(range(2, 3))
